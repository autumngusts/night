...
| 復仇者の靈體/死靈が亂戰・個別傷害を受ける | ✅ 2026-08-22実装、同日中に配分方式を再修正。従来`#enemy-damage-modal`にはPC行しかなく、召喚中の靈體（`c.spiritSummon`）や死靈術で召喚した死靈（`c.deathSpirits`）は傷害を與えるだけで受けることが一切できなかった（本文「靈體會分擔以『前衛』為目標的亂戰傷害（相當於1名PC份），個別傷害則被視為『敵視：0』的1名PC，且不會受到傷害以外的任何效果」が未実装）。PC行と同じ見た目・同じ`floor((亂戰+個別)/HP價值)`計算式の行を、entered中の全復仇者が持つ靈體/死靈ごとに追加（HP價值は固定値：海倫/死靈=80、弗雷德里克=60、賽巴斯汀=80）。個別傷害の自動対象選択は靈體を対象にしない＝GMが必要な場合のみ手動入力する設計。「傷害以外の効果を受けない」に対応するため、靈體/死靈専用の確定処理（`handleEnemyDamageConfirmForSpirit`）はPC専用の遺物効果修正（斧槍旋風／完全無効化等）や屬性/異常蓄積の反映を一切行わない。**亂戰傷害の配分方式（2026-08-22再修正、使用者確認）**：初回実装はPC側の人数・分配（`groupTargets`／`shares`）をそのままにして、その回の亂戰傷害の対象に前衛のPCが1人以上含まれていた場合のみ靈體/死靈にPCと同額の1人分を「追加で」渡していた（＝傷害池の総量が水増しされる）。これは「靈體與PC共通分攤亂戰傷害」の字面（同じ池を分け合う）と矛盾するとの指摘を受け、`AutoGm.splitGroupShares`へ渡す人数を`groupTargets.length + spiritSplitTargets.length`（対象PC数＋対象靈體/死靈数）に変更し、同じ傷害池を全員で均等割りする方式へ修正した（トリガー条件＝対象PCに前衛が1人以上いることは維持） | `night.js`の`activeSpiritDamageTargets`, `handleEnemyDamageConfirmForSpirit`, `checkAllEnemyDamageRowsConfirmedAndFinish`, `renderEnemyDamageModal`, `handleAutoGmRollClick`, `autoTriggerDefenseRoll` |
| 遺物効果「致命一擊」の使用制限単位（1回合→1階段） | ✅ 2026-08-22発見・修正。本文「1回合僅限1名PC使用」に基づき`state.battle.fatalStrikeUsedThisRound`という単一ロックで実装済みだったが、そのロックを解除する処理が実装のどこにも無く、実際には「一度使われたら、その戰鬥が終わるまで誰も使えない」という過剩に厳しい挙動になっていた。使用者確認により、制限の単位は実際には「1階段（戰鬥階段／額外階段）につき全PC共通で1回」であるべきと判明したため、`setActionPhase()`でcombat／extraへ新規突入するたび（同じ回合内でのcombat⇔extra間の遷移も含む）にこのロックを解除するよう修正した | `night.js`の`setActionPhase`, `renderFatalStrikeAction` |
| 「已完成」按鈕列の取消（取消準備）が実行行動記錄まで削除してしまう不具合 | ✅ 2026-08-22発見・修正。2026-08-22の前回修正（上記「実行行動記錄の取消と瀕死角色への復歸傷害転換」行）で、「已完成」を取消したとき角色の`c.pendingActionBoxes`（執行行動記錄／点線枠）を全件`removeActionBox`で削除し、瀕死角色がいれば逐一「復歸傷害へ転換するか」を確認するダイアログを出す実装にしていた。これは「取消準備」という操作の意図（単に準備完了フラグを戻すだけ）を超えて実際の傷害記錄を破壊してしまうバグだった。使用者確認により、「取消已完成」は`state.battle.roundActionsDone[c.id]`と`state.battle.roundActionLog`（進度版の表示順序）のみを操作し、`c.pendingActionBoxes`には一切触れない（削除も転換確認も行わない）よう修正した。単一の執行行動を削除する場合の復歸傷害転換確認は、引き続き執行行動記錄自身の右上Xボタン（`removeActionBox`→`promptConvertDamageToRevival`）のみが担う | `night.js`の`renderBattleRoundActionButtons`内の「已完成」按鈕クリックハンドラ、`removeActionBox`, `promptConvertDamageToRevival` |
