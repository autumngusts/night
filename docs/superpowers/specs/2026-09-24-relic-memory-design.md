...
## 10.20 抽選規則補充、固定配置遺物發放、不可疊加標示（2026-09-25）

### 使用者明確規格（原文要旨）

> 同一id不會出現兩個在同一遺物記憶中
> 特定遺物記憶（如打過某劇本王的獲得品）…擊破三頭犬必定另外獲得 存入db 有相同物品則不儲存兩件 自動訂為最愛
> 一顆記憶只會出現最多一條：全局里程碑（7 條）、戰技置換（20 條）、武器屬性附加（7 條）、出擊時道具（22 條）、
> 出擊時聖杯瓶（22 條）、魔術祈禱置換、盧恩（3 條）
> 不可疊加：標示「不可疊加，此條未發動」，等待房勾選清單也用同一個判斷。帶入順序是勾選順序、勾選時標註①②③

### 同 id 不重複

`rollEffects()` 抽到的本來就會從候選移除；另外在入口把傳入池本身的重複 id 去掉，兩道保證。

### 互斥組（只限制抽選）

| 組 | 內容 | 帶入時「同組只發動第一條」 |
| --- | --- | --- |
| `startWeapon` | 武器屬性附加 7 ＋ 戰技置換 20 ＋ 魔術祈禱置換 10（合為一組） | 是 |
| `crystalTear` | 出擊時聖杯瓶（結晶雫）22 | 是 |
| `milestone` | 全局里程碑 7（含小砦盧恩那條） | 否 |
| `startItem` | 出擊時道具 22 | 否 |
| `rune` | `rm_critical_rune`／`rm_shop_discount`／`rm_party_rune_up`（kind 是 special，以 id 指定） | 否 |

- 「出撃時の武器」三類這次分開列出，但前一次規格是合為一組（§10.17），合為一組同時滿足「每類最多一條」，因此維持。
- 新的三組只限制抽選；帶入多顆時依各自的 `stackable` 處理（catalog `loadoutFirstOnlyGroup()`）。

### 固定配置遺物的發放

- 條件：以劇本 X 擊敗第 3 天夜王（`day3BossDefeated()` ＋ `meta.resolvedNightBossId`）。對應表在
  catalog 的 `FIXED_RELIC_BY_SCENARIO`（10 顆：9 顆「出撃○○で3日目ボス」＋「安寧者たち をクリア」＝劇本 balancers 通關）。
- 發不出的 8 顆：「常夜の王○○」6 顆（midnight 沒有常夜之王）與「マルギット撃破」（midnight 沒有此敵人）。
- 流程：`meta/relicMemoryGrants/fixed = { kind: "fixed", fixedId }`（first-writer-wins），各裝置以
  `RM.newFixedMemory()` 產生：效果是指定的、有 range 的照樣擲值、強化版只擲後段；`size` 依條數（2＝中、3＝大），
  帶 `fixedId`、`favorite: true`。
- 存入：`mergeIntoStore()` 遇到 store 已有相同 `fixedId` 的記憶就不存（`duplicateFixed` 計數，結算狀態附註）；
  存入的固定遺物一律 `favorite: true`。
- 顯示：大小後面接遺物名（「中・獸之夜」）；等待房選擇時 `fixedId` 一併複製進 loadout。
- 固定遺物上 4 條沒有規格也沒接入的效果（`rm_item_effect_to_allies`／`rm_melee_atk_up`／`rm_weapon_skill_atk_up`／
  `rm_ailment_gauge_atk_up`，§10.6.1）照樣發放，顯示時加註「此效果尚未實作，僅顯示」（同日稍後已接入，見 §10.21）。

### 不可疊加標示與①②③

- `relicMemoryEffectDuplicated(c, memId, id)`：`stackable:false` 且在「習得附帶效果 → 帶入順序 → 記憶內順序」中
  前面已出現過 → 未發動。跟 `relicMemoryBonusTotal()`／`activeAttachedEffectIds()` 的「取第一次出現」一致。
- 標示共用 `relicMemoryEffectStatusNote()`：尚未實作／同類只發動第一條／不可疊加，用在角色視窗詳細與等待房。
- 帶入順序＝等待房勾選順序；等待房勾選中的列與角色視窗格子標註①②③。

### 驗證

- `relic_memory_unit_check.js`：同 id、五組條數與「每組最多一條」（大記憶 20000 顆）、固定遺物的產生／強化版／存入去重／自動最愛。
- `relic_memory_full_flow_check.js`：劇本三つ首の獣擊敗夜王 → 得「獣の夜」且自動最愛；DB 已有同一件 → 不存第二件；
  等待房①②③與「不可疊加」標示、HP 上限只算第一顆、角色視窗①②③與詳細標示。

## 10.21 固定配置遺物上 4 條效果的接入（2026-09-25）

### 使用者明確規格（原文）

> 「アイテム効果が周囲の味方にも」 使用道具時 同一戰場的友軍獲得20%效果
> 「近接攻撃力上昇」 近距離攻擊力上升 8~12%
> 「戦技攻撃力上昇」 戰技攻擊力上升 8~12%
> 「異常ゲージ攻撃力上昇」 身負異常狀態時攻擊力上升8~12%

這 4 條就是 §10.6.1 補建、原本 note 空、range null 的那 4 條；§10.20 顯示「此效果尚未實作」的也是它們，接入後不再顯示該註記。
仍是 `relicOnly`（只出現在固定配置遺物上），不進抽選池。`stackable` 使用者沒有指定，維持 null（＝預設不可疊加）。

| 效果 | 範圍 | 注入點 |
| --- | --- | --- |
| 戦技攻撃力上昇 | 8~12% | alias 併進既有詞條 `weaponArtUp`（`ctx.art`），語意相同 |
| 近接攻撃力上昇 | 8~12% | `affixOutgoingDamageMult()`：`ctx.weaponId` 存在、非遠程武器（`isRangedWeapon()`，蓄力／跳躍的呼叫端沒傳 `ctx.ranged`，因此直接看武器）、非魔術／祈禱。含近戰武器的戰技 |
| 状態異常ゲージがある時、攻撃力上昇 | 8~12% | 同上；條件 `selfAilmentGaugeActive()`＝自身 7 種異常任一蓄積值 >0 |
| アイテムの効果が周囲の味方にも発動 | 固定 20% | `shareConsumableWithEncounterAllies()`／`applyRelicMemorySharedItemEffect()` |

### 「身負異常狀態」的對應

midnight 的異常是蓄積到門檻瞬間觸發（扣血等）後歸零，沒有持續的「異常狀態」。因此以蓄積計量表為準（目錄原名「状態異常ゲージがある時」）：
自身蓄積列上有異常徽章時成立。屬性（炎魔雷聖）不算異常。原名的「徐々に（逐漸）」使用者換算沒有，照換算一次到位。

### 道具效果分給友軍（20%）

- 「同一戰場」＝目前遭遇的 `participants`（不含自己）；不在戰鬥中沒有對象。
- 傳遞沿用學者「道具效果擴大」的模式：寫 `character/<對方token>/_rmSharedItemEffect = { itemId, at, scale: 0.2, level2 }`，
  由對方自己的裝置套用（FP／體力／本地 buff 只有對方寫得進去）。另開欄位，避免與 `_sharedItemEffect` 同時發生時互相覆蓋；
  第一次收到快照只建立基準（reload 不會重套）。
- 「20% 效果」只對**友軍自己受益**的道具定義，數值 ×0.2 四捨五入：

| 道具 | 友軍得到 |
| --- | --- |
| 星光の欠片 | FP 30／60 → 6／12 |
| 温石 | 自身 10 秒持續回復 20／40 → 4／8（不再轉給第三人） |
| 亀首の漬物 | 體力 10 → 2；等級 2 HP 回復 20 → 4 |
| 苦薬 | 同樣清除自身 1 種異常蓄積（使用者 2026-09-25 明確規格，不按比例）；等級 2 HP 回復 20 → 4 |
| 勇者の肉塊 | 攻擊 +5／+10、戰技 +5 → +1／+2、+1；等級 2 HP 回復 → 4 |
| 酸の噴霧 | 敵傷害 −12 → −2；等級 2 HP 價值 +10 → +2 |
| 鉄壺の香薬 | 「不承受 HP 損害」→ HP 損害 −20%（代價的體力扣除不套用在友軍） |

- 不分享：攻擊敵人的道具（投擲類、噴霧的蓄積與雜兵傷害）不是「友軍獲得」的效果；高揚之香本來就是全隊效果；塗脂塗在使用者自己的武器上。
- buff 的比例存在本地欄位 `_heroMeatScale`／`_acidSprayScale`／`_ironPotScale`（列入 `LOCAL_ONLY_CHARACTER_FIELD_RE`），
  自己使用時設回 1；友軍分享來的 20% **不會覆蓋**自己仍在時效內、比例較高的同一個 buff。
- 等級 2 依使用者那一方的判定（`payload.level2`）。

### 驗證

- `relic_memory_effect_check.js`：近接（近戰一般攻擊／近戰戰技 ×1.10、弓／魔術／非武器不加）、戦技（×1.09、一般攻擊不加）、
  異常計量表（無蓄積不加、猛毒蓄積後 ×1.11）、4 條已接入且不在抽選池。依 §4.7 更新：alias「全部可抽選」排除 relicOnly、
  累計已接 361→365、「仍僅顯示」改為直接數未接條數。
- `relic_memory_full_flow_check.js`（emulator，兩台裝置）：A 使用勇者の肉塊 → B 得 +1／+2／+1；酸の噴霧 → −2；
  沒帶記憶／投擲壺／只有自己時不分享；鐵壺 20% ＝不免傷、−20%；B 自己的完整版不會被 20% 降級。
- `relic_memory_unit_check.js`：依 §4.7 更新（range 條數 156→159、4 條不再是「note 空、range null」）。
