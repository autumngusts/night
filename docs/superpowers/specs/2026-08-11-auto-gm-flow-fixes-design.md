...
追加調査（2026-08-11、ユーザー報告）：「戰鬥の額外階段で敵を擊破・獎勵処理後、GMが『此樓層已無東西』的な表示を出し、踏破判定・地圖移動の案内なしで別の樓層敘述の頭へ戻ってしまうことがある」というバグ報告があり、Playwrightで複数パターン（單/多段HP敵、單/2樓層カード、正規の額外階段遷移）を再現テストしたが確定再現には至らなかった。ただし報告元ゲームは `storageMode: "cloud"` であり、本セッションの前段で既に修正した別バグ（`static_src/night.js` の `loadState()`/`applyLoadedData()` の `actionKind` ホワイトリストに `"floorEnd"`/`"mapMove"` が漏れていた——他端末のFirebase同期のたびに `applyLoadedData` が再実行され、獎勵ゲート・地圖移動ゲートが黙って`"ok"`へ巻き戻る、commit `4eb2597`）が原因として最も整合する。追加のコード変更は本設計に含めない（既存修正でカバー済みと判断）。再発する場合は改めて調査する。
