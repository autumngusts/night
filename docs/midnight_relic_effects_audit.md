...
**2026-09-11 更新：經過 09-10／09-11 兩批補實作後，353 筆中只剩 25 筆尚未接上**
（逐筆清單見 `docs/midnight_relic_effects_gaps_by_type.md`，可用
`npm run audit:relic_gaps` 重新產生）。依生效路徑分類：

| 分類 | 筆數 |
| --- | --- |
| midnight.js 直接以名稱判斷（含 2026-09-11 新增的大批效果） | 266 |
| 通用被動解析（威力補正／最大HP・FP／判定骰） | 37 |
| 2Hit攻擊的達人（消耗覆寫，冷却10秒） | 16 |
| `variantEntry`（角色面板可切換的替代招式） | 8 |
| CharacterDrawer 內建（發現力＋） | 1 |
| **尚未接上** | **25** |

剩下的 25 筆幾乎都是暗黑／黎明變體的專屬效果（使用者的清單沒有列到），
以及少數在即時制下結構上不成立的回合制概念（體力骰帶入、「下個防禦階段」等）。
