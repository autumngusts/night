    ...
    "undead|falling_hawk_corps": {
      rows: [
        ...
        },
        {
          ...
          // 「潜航＆毒液」で新設）を再利用する。【レビューで確認済み】本文は「『敵視:最大』のPCが
          // どちらのエリアにも同数存在する場合、通常どおり、前衛が対象となる」と明記しており、
          // gnoster／nameless「転移＆錫杖薙ぎ払い」／fulghor「とびかかり＆氷霜」の3体（規則書原文が
          // 「同数の場合はランダムに決定する」と明記）とは異なる、実在する規則書間の矛盾であることが
          // 確認された。共有ロジック（auto_gm.jsのresolveTargets）に任意の`tieBreak`パラメータを
          // 追加し、この敵のみ`tieBreak: "front"`を明示指定することで規則書どおり前衛固定とする
          // （tieBreak未指定の既存3体はランダム挙動のまま、影響なし）。
          rollMin: 5,
          rollMax: 6,
          groupDamage: { modifier: 0 },
          targetRule: { kind: "majorityAreaAggroMax", tieBreak: "front" },
        },
      ],
    },
    "undead|skeletons": {
